(() => {
  ...
  const replacements = {
    ...
    ko: [
      [/(\d+) GPU presets or Custom/g, "GPU 프리셋 $1개 또는 직접 입력"],
      [/(\d+) measurements · median/g, "실측 $1건 · 중앙값"],
      [/Calculated estimate · expected error/g, "계산 추정 · 예상 오차"],
      [/Available VRAM/g, "가용 VRAM"],
    ],
  };

  function apply(language = document.documentElement.lang) {
    const exact = new Map(language === "en" ? exactPairs : exactPairs.map(([ko, en]) => [en, ko]));
    document.querySelectorAll("body *:not(script):not(style):not(code):not(pre)").forEach((node) => {
      ...
      replacements[language === "en" ? "en" : "ko"].forEach(([pattern, next]) => { value = value.replace(pattern, next); });
