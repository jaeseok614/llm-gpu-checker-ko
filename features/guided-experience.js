(() => {
  const COPY = {
    ko: {
      ...
      requestLinks: ["GPU 추가 요청", "모델 추가 요청", "국내 가격 제보"],
    },
    en: {
      ...
      requestLinks: ["Request a GPU", "Request a model", "Report Korean pricing"],
    },
  };
  ...
  function bind() {
    ...
    window.addEventListener("ai-hardware-workspacechange", (event) => render(event.detail?.mode));
  ...
  window.AIHardwareGuide = { bind, render, setLanguage, toggleGuide };
