...
6. 모델 상세의 `동시 처리 용량 (베타)`에서 권장·최대 동시 인원과 처리량을 확인하거나, 결과 상단의 `여러 GPU에 모델 동시 배치 추천 (베타)`에서 여러 모델의 배치 조합을 계산합니다.
...
| 여러 GPU 모델 배치 (베타) | 서로 다른 GPU 여러 대와 여러 생성형 LLM을 선택하면 GPU별 모델·양자화 배치 및 배치 불가 모델을 계산 |
