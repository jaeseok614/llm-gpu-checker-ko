...
- **v7.21.0** — "API vs Local" 탭에 실제 Local(자체 구축) 비용을 추가했습니다. 등급별 대표 모델·기준 GPU·양자화(저가형: Qwen3 8B+RTX 4060 Ti, 균형형: Qwen2.5 32B+RTX 5090, 플래그십: Llama 3.3 70B+RTX 6000 Ada)로 GPU 대수·구매비·전기료·유지비를 반영한 3년 분할 월 비용을 계산해, 같은 등급 API 최저가와의 차액·회수 개월 수까지 보여줍니다.
