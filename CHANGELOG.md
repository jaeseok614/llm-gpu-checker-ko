...
- 생성형 LLM 147종, 임베딩 60종, 리랭커 34종, OCR/VLM 45종 등 등록된 모델 286개 전체에 실제 영문 요약 데이터 추가. 지금까지 영어 모드에서는 모델별 한국어 설명 대신 "language model for {태그}. The Korean description is available in the Korean interface." 같은 일반 문장으로 대체됐는데, 이제 `summary` 필드를 `{ ko, en }` 형태로 바꾸고 기존 한국어 설명을 그대로 번역해 넣어 영어 사용자도 모델별로 실제 설명을 봄. 데이터 유효성 검사(`npm run check`의 validate-data)와 jsdom 검증으로 286개 전부 객체 형태로 반영됐는지, 언어 전환 시 정상적으로 전환되는지 확인
