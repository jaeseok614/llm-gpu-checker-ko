...
const serialized = `{ name: ${JSON.stringify(record.name)}, provider: ${JSON.stringify(record.provider)}, params: ${record.params}, active: ${record.active}, license: ${JSON.stringify(record.license)}, language: ${JSON.stringify(record.language)}, summary: ${JSON.stringify(record.summary)}, sourceUrl: ${JSON.stringify(record.sourceUrl)}, licenseUrl: ${JSON.stringify(record.licenseUrl)}${workload.startsWith("audio-") ? `, type: ${JSON.stringify(workload)}` : ""} }`;
